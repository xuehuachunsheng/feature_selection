package ufs.general.nnedls.test;
...
import ufs.featureselection.impl.NNEdls;
...
public class TestNnedls {
	public static void main(String[] args) throws Exception {
		System.out.println("NNEDLS");
		...
		for (int i = 0; i < ConstValues.STD_DATA_MATRIX.length; i++) {
			...
			NNEdls edls = new NNEdls(X, neighbors.selectColumns(Ret.NEW,
					numNeighbors), 0);
			edls.middleProcess();
			...
			for (int numFeatures = 20; numFeatures <= 200; numFeatures += 20) {
				...
				for (int j = 0; j < numRepeat; j++) {
					Cluster cluster = new KMeansInWeka(
							edls.getDataAfterFeaturesSelected(), numClusters);

					cluster.setRealLabels(Y.transpose().toIntArray()[0]);
					cluster.cluster();

					tACCSum += cluster
							.getEvaluationResult(EvaluationIndexType.ACC);
				}
